...
class RSI20(bt.Strategy):
    ...
    def __init__(self):
        ...
        self.rsi = myind.RSIWithSMA(period=20)
        self.ema = bt.indicators.EMA(period=200)
        self.atr_sl = myind.ATRStopLoss(self.datas[0], period=14, multiplier=2)
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if self.datas[0].close[0] > self.ema[0] and self.rsi.rsi[0] > self.rsi.sma[0]:
            ...
            if self.entry_type == 'buy':
                ...
                if self.rsi.rsi[0] < self.rsi.sma[0] and self.datas[0].close[0] > self.entry_price:
